...
class Fund(models.Model):
    ...
    latest_nav = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="最新净值"
    )
    latest_nav_date = models.DateField(null=True, blank=True, help_text="最新净值日期")

    # 实时估值数据（缓存）
    estimate_nav = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, help_text="实时估值净值"
    )
        ...
        if self.is_default:
            ...
            if existing_default:
                raise ValidationError(
                    f"用户 {self.user.username} 已有默认账户：{existing_default.name}"
                )
